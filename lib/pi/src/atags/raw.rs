...
impl Atag {
    ...
    pub fn next(&self) -> Option<&Atag> {
        if self.tag == Atag::NONE { // if this tag is the last tag
            return None; // Can't get next, return None
        } else { // not the last tag, Ok to move forward
            let self_tag = self as *const Atag as *const u32;
            unsafe {
                let next_tag = self_tag.offset(self.dwords as isize) as *const Atag;
                return Some(&*next_tag);
            }
        }
    }
}
