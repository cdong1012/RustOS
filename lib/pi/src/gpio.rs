...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        let register_num = (self.pin / 10) as usize;
        let bits = ((self.pin % 10) * 3) as usize;

        self.registers.FSEL[register_num].and_mask(!(0b111 << bits)); // clear the function select

        self.registers.FSEL[register_num].or_mask((function as u32) << bits); // set function
        self.transition()
    }
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        ...
        self.registers.SET[register_num].write(0b1 << ((self.pin % 32) as usize));
    }

    /// Clears (turns off) the pin.
    pub fn clear(&mut self) {
        let register_num = (self.pin / 32) as usize;
        self.registers.CLR[register_num].write(0b1 << ((self.pin % 32) as usize));
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        let register_num = (self.pin / 32) as usize;
        self.registers.LEV[register_num].has_mask(1 << ((self.pin % 32) as usize))
    }
}
