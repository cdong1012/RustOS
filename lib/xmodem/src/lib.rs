...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let read_result = self.read_byte(false)?;
        if read_result == byte { // match
            Ok(read_result)
        } else { // not match, write CAN to inner stream
            self.write_byte(CAN)?;
            match read_result {
                CAN     => return ioerr!(ConnectionAborted, "receive CAN"),
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 { return ioerr!(UnexpectedEof, "buffer length is less than 128"); }
        if self.started == false { // if the transmission hasnt started
            self.write_byte(NAK)?; // we send a NAK to initiate the transmission
            self.started = true;
            (self.progress)(Progress::Started); // start progress
        }

        // now, we will wait to receive a SOH from the sender.
        
        let first_byte = self.read_byte(true)?; // if CAN is received unexpectedly, return error
        if first_byte == SOH { // transmission
            self.expect_byte_or_cancel(self.packet, "expected packet number")?;
            self.expect_byte_or_cancel(255u8.wrapping_sub(self.packet), "expected packet number in 1's comp")?;
            self.inner.read_exact(buf)?; //read a packet into buf
            let original_checksum = self.read_byte(false)?;
            let checksum : u8 = get_checksum(buf);
            if original_checksum != checksum {
                self.write_byte(NAK)?;
                return ioerr!(Interrupted, "packet interrupted, checksum mismatched");
            }
            self.write_byte(ACK)?; // send back ACK
            // signal packet transmitted 
            (self.progress)(Progress::Packet(self.packet));
            self.packet = self.packet.wrapping_add(1); // add 1 to the packet num
            Ok(128) // return
        } else if first_byte == EOT { // end of transmission
            self.write_byte(NAK)?;
            ...
            return ioerr!(InvalidData, "Data not EOT or SOH");
        }
    }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if buf.len() != 128 && buf.len() != 0 {
            return ioerr!(UnexpectedEof, "packet size not valid");
        }
        if !(self.started) {
            (self.progress)(Progress::Waiting); // wait for NAK
            self.expect_byte(NAK, "expecting NAK for first packet.")?;
        ...
        if buf.len() == 0 { // EOT
            self.write_byte(EOT)?;
            self.expect_byte(NAK, "expecting NAK")?;
            self.write_byte(EOT)?;
            self.expect_byte(ACK, "expecting ACK")?;
            self.packet = self.packet.wrapping_add(1);
            return Ok(0);
        } else if buf.len() == 128 {
            let checksum : u8 = get_checksum(buf);
            ...
            self.write_byte(255u8.wrapping_sub(packet_num))?;
            self.inner.write_all(buf)?; // send the whole packet

            self.write_byte(checksum)?;
            let send_result = self.read_byte(true)?;

            match send_result {
                NAK => return ioerr!(Interrupted, "interrupted"),
                ACK => {
                    self.packet = self.packet.wrapping_add(1);
                ...
                _   => {
                    return ioerr!(InvalidData, "expected NAK or ACK");
                }
            }
        }
        return ioerr!(InvalidData, "invalid end data");
    } 
