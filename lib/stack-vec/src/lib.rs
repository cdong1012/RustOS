...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn new(storage: &'a mut [T]) -> StackVec<'a, T> {
        StackVec {
            storage: storage,
            len: 0 // empty so the length is 0
        }
    }
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len > storage.len() { // if length
            panic!("length is too long compared to storage size");
        }
        StackVec { 
            storage: storage,
            len : len
...
impl<'a, T: Clone + 'a> StackVec<'a, T> {
    ...
    pub fn pop(&mut self) -> Option<T> {
        if self.is_empty() {
            ...
            let return_result : T = (self.storage[self.len - 1]).clone();
            self.len -= 1;
            Some(return_result)
        }
    }
